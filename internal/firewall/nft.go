...
// nftConf is the temp file written before `nft -f`
...
// -------------------------------------------------------------------
// tproxy mode  (strictly ported from v2rayA nftTproxy.GetSetupCommands)
// -------------------------------------------------------------------

func setupTproxy(port int, lanProxy bool, ipv6 bool) error {
	table := buildTproxyTable(port, ipv6)
	if err := os.WriteFile(nftConfPath, []byte(table), 0644); err != nil {
		return fmt.Errorf("write nft conf: %w", err)
	}

	// ip rule / ip route  (fwmark → local routing table 100)
	routeCmds := []string{
	...
	for _, c := range routeCmds {
		if err := runCmd(c); err != nil {
			log.Printf("firewall: route cmd warning: %v", err)
		}
	}

	if lanProxy {
		if err := enableIPForward(ipv6); err != nil {
			log.Printf("firewall: ip_forward warning: %v", err)
		}
	}
...
func buildTproxyTable(port int, ipv6 bool) string {
	...
	tproxyV6 := fmt.Sprintf("        meta l4proto { tcp, udp } mark & 0xc0 == 0x40 tproxy ip6 to [::1]:%d\n", port)
	if !ipv6 {
		tproxyV6 = ""
	}
...
`, port, tproxyV6, nfproto, nfproto)
	return table
}

// -------------------------------------------------------------------
// redirect mode  (strictly ported from v2rayA nftRedirect.GetSetupCommands)
// -------------------------------------------------------------------

func setupRedirect(port int, lanProxy bool, ipv6 bool) error {
	table := buildRedirectTable(port, ipv6)
	if err := os.WriteFile(nftConfPath, []byte(table), 0644); err != nil {
		return fmt.Errorf("write nft conf: %w", err)
	}
...
table inet v2raya {
    ...
    chain tp_pre {
        ...
        %s meta l4proto tcp jump tp_rule
    }
...
`, port, nfproto, nfproto)
}

// -------------------------------------------------------------------
// Cleanup  (called on any shutdown)
// -------------------------------------------------------------------
...
func cleanupTproxyRoutes() {
	...
	for _, c := range cmds {
		_ = runCmd(c) // ignore errors — may not exist
	}
}

// -------------------------------------------------------------------
// Watcher integration: add/remove IPs from nftables sets
// -------------------------------------------------------------------
...
func AddInterfaceIP(cidr string) {
	...
	cmd := fmt.Sprintf("nft add element inet v2raya %s { %s }", setName, cidr)
	if err := runCmd(cmd); err != nil {
		log.Printf("firewall: add interface IP %s: %v", cidr, err)
	}
}
...
func RemoveInterfaceIP(cidr string) {
	...
	cmd := fmt.Sprintf("nft delete element inet v2raya %s { %s }", setName, cidr)
	if err := runCmd(cmd); err != nil {
		log.Printf("firewall: remove interface IP %s: %v", cidr, err)
	}
}
...
// IsIPv6Supported checks whether the system has a working IPv6 loopback.
